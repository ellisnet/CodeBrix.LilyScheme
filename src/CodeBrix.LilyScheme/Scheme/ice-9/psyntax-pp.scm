...
(let ((syntax? (module-ref (current-module) 'syntax?))
  ...
  (letrec* ((make-void (lambda (src) (make-struct/simple (vector-ref %expanded-vtables 0) src)))
            ...
            (expand-macro
             (lambda (p e r w s rib mod)
               (letrec* ((decorate-source (lambda (x) (source-wrap x empty-wrap s #f)))
                 ...
                 (let* ((t-680b775fb37a463-1b3a transformer-environment)
                        (t-680b775fb37a463-1b3b (lambda (k) (k e r w s rib mod))))
                   (with-fluid*
                    t-680b775fb37a463-1b3a
                    t-680b775fb37a463-1b3b
               ...
               (letrec* ((key->keyword
                          (lambda (key)
                            (let ((key (syntax->datum key)))
                              (cond
                                ((memv key '(#:select :select)) #:select)
                                ((memv key '(#:hide :hide)) #:hide)
                                ((memv key '(#:prefix :prefix)) #:prefix)
                                ((memv key '(#:renamer :renamer)) #:renamer)
                                (else #f)))))
                         (check-spec-arguments
                          (lambda (arguments)
                            (letrec* ((valid-select-spec?
                                       (lambda (spec)
                                         (let* ((tmp-1 spec) (tmp (list tmp-1)))
                                           (if (and tmp (apply (lambda (symbol) (id? symbol)) tmp))
                                               (apply (lambda (symbol) #t) tmp)
                                               (let ((tmp ($sc-dispatch tmp-1 '(any . any))))
                                                 (and (and tmp
                                                           (apply (lambda (orig seen) (and (id? orig) (id? seen))) tmp))
                                                      (apply (lambda (orig seen) #t) tmp))))))))
                              (let* ((tmp-1 arguments) (tmp ($sc-dispatch tmp-1 '())))
                                (if tmp
                                    (apply (lambda () #t) tmp)
                                    (let ((tmp ($sc-dispatch tmp-1 '(any any . any))))
                                      (and tmp
                                           (apply (lambda (key value rest)
                                                    (let ((key (key->keyword key)))
                                                      (cond
                                                        ((memv key '(#:select))
                                                         (let* ((tmp value) (tmp ($sc-dispatch tmp 'each-any)))
                                                           (and tmp
                                                                (apply (lambda (spec)
                                                                         (and (and-map valid-select-spec? spec)
                                                         ...
                                                         (let* ((tmp value) (tmp ($sc-dispatch tmp 'each-any)))
                                                           (and tmp
                                                                (apply (lambda (id)
                                                                       ...
                                                                       tmp))))
                                                        ((memv key '(#:prefix))
                                                         (and (id? value) (check-spec-arguments rest)))
                                                        ((memv key '(#:renamer)) (check-spec-arguments rest))
                                                        (else #f))))
                                                  tmp))))))))
                         (get-kw-argument
                          (lambda (arguments transform kw)
                            (let extract ((arguments arguments) (last-value #f))
                              (let* ((tmp arguments) (tmp-1 ($sc-dispatch tmp '(any any . any))))
                                (if tmp-1
                                    (apply (lambda (key value rest)
                                             (extract
                                              rest
                                              (if (eq? (key->keyword key) kw) (transform value) last-value)))
                                           tmp-1)
                                    (let ((tmp-1 ($sc-dispatch tmp '())))
                                      (if tmp-1
                                          (apply (lambda () last-value) tmp-1)
                                          (syntax-violation #f "source expression failed to match any pattern" tmp))))))))
                         (make-selection
                          (lambda (select-specs)
                            (map (lambda (sym-or-pair)
                                   (if (symbol? sym-or-pair) (cons sym-or-pair sym-or-pair) sym-or-pair))
                                 select-specs)))
                         (validate-select-specs
                          (lambda (module-name interface specs)
                            (let* ((tmp-1 specs) (tmp ($sc-dispatch tmp-1 'each-any)))
                              (if tmp
                                  (apply (lambda (spec)
                                           (for-each
                                            (lambda (spec)
                                              (let* ((tmp-1 spec) (tmp (list tmp-1)))
                                                (if (and tmp (apply (lambda (id) (id? id)) tmp))
                                                    (apply (lambda (id)
                                                    ...
                                                    (let ((tmp ($sc-dispatch tmp-1 '(any . any))))
                                                      (if (and tmp
                                                               (apply (lambda (orig seen) (and (id? orig) (id? seen)))
                                                                      tmp))
                                                          (apply (lambda (orig seen)
                                                                   (if (not (module-local-variable
                                                                             interface
                                                                             (syntax->datum orig)))
                                                                       (syntax-violation
                                  ...
                                  (syntax-violation #f "source expression failed to match any pattern" tmp-1)))))
                         (validate-hide-specs
                          (lambda (module-name interface ids)
                            (let* ((tmp-1 ids) (tmp ($sc-dispatch tmp-1 'each-any)))
                              (if tmp
                                  (apply (lambda (id)
                                           (for-each
                                            (lambda (id)
                                              (if (not (module-local-variable interface (syntax->datum id)))
                                                  (syntax-violation
                                                   'with-modules
                                                   (simple-format
                                                    #f
                                                    "no binding `~A' to hide in module ~A"
                                                    (syntax->datum id)
                                                    module-name)
                                                   id)))
                                            id))
                         ...
                         (collect-module-bindings
                          (lambda (module-name interface select hide prefix renamer)
                            (letrec* ((selector
                                       (if select
                                           (let ((selection (make-selection select)))
                                             (lambda (x) (assq-ref selection x)))
                                           identity))
                                      (hide? (if hide (lambda (x) (and (memq x hide) #t)) (const #f)))
                                      (rename
                                       (cond
                                         (renamer (eval-local-transformer (expand renamer r w mod) mod))
                                         (prefix
                                          (let ((prefix (symbol->string prefix)))
                                            (lambda (x) (string->symbol (string-append prefix (symbol->string x))))))
                                         (else identity)))
                                      (core-@ (make-syntax '@ top-wrap '(hygiene guile) #f))
                                      (make-module-ref
                                       (lambda (id module-name orig) (datum->syntax id (list core-@ module-name orig))))
                                      (make-module-variable-transformer
                                       (lambda (module-name orig)
                                          ...
                                          (lambda (x)
                                            (let* ((tmp x)
                                                   (tmp-1 ($sc-dispatch
                                                           ...
                                                           (list (vector
                                                                  'free-id
                                                                  (make-syntax 'set! '((top)) '(hygiene guile)))
                                                                 'any
                                                                 'any))))
                                              (if tmp-1
                                                  (apply (lambda (id val)
                                                           (syntax-violation
                                                            'set!
                                                            "cannot assign to a binding imported via with-modules"
                                                            (source-wrap e w s mod)
                                                            x))
                                                         tmp-1)
                                                  (let ((tmp-1 ($sc-dispatch tmp '(any . any))))
                                                    (if tmp-1
                                                        (apply (lambda (id args)
                                                                 (let* ((tmp (make-module-ref id module-name orig))
                                                                        (ref tmp))
                                                                   (cons ref args)))
                                                               tmp-1)
                                                        (let ((tmp-1 (list tmp)))
                                                          (if (and tmp-1 (apply (lambda (id) (identifier? id)) tmp-1))
                                                              (apply (lambda (id) (make-module-ref id module-name orig))
                                                                     tmp-1)
                                                              (syntax-violation
                                                               #f
                                                               "source expression failed to match any pattern"
                              ...
                              (let ((bindings '()))
                                (module-for-each
                                 (lambda (orig var)
                                   (let ((seen (selector orig)))
                                     (if (and seen (not (hide? orig)))
                                         (let ((id (make-syntax (rename seen) empty-wrap mod #f))
                                               (binding
                                                (cons 'macro
                                                      (if (and (variable-bound? var) (macro? (variable-ref var)))
                                                          (macro-binding (variable-ref var))
                                                          (make-module-variable-transformer module-name orig)))))
                                           (set! bindings (cons (cons id binding) bindings))))))
                                 interface)
                                bindings))))
                         (valid-module-name?
                          (lambda (stx)
                            (let* ((tmp stx) (tmp ($sc-dispatch tmp '(any . each-any))))
                              (and tmp
                                   (apply (lambda (part parts)
                                            (and (and-map id? (cons part parts))
                                                 (and-map symbol? (syntax->datum (cons part parts)))))
                                          tmp)))))
                         (normalize-spec
                          (lambda (spec)
                            (let* ((tmp-1 spec) (tmp ($sc-dispatch tmp-1 '(any . each-any))))
                              (if (and tmp (apply (lambda (part parts) (valid-module-name? spec)) tmp))
                                  (apply (lambda (part parts) (datum->syntax spec (list spec))) tmp)
                                  (let ((tmp ($sc-dispatch tmp-1 '(any . any))))
                                    (and (and tmp
                                              (apply (lambda (module-name arguments) (valid-module-name? module-name))
                                                     tmp))
                                         (apply (lambda (module-name arguments)
                                                  (and (check-spec-arguments arguments) spec))
                                                tmp)))))))
                         (collect-all-bindings
                          (lambda (module-specs)
                            (let loop ((specs module-specs) (all-bindings '()))
                              (if (null? specs)
                                  all-bindings
                                  (let* ((tmp-1 (normalize-spec (car specs))) (tmp ($sc-dispatch tmp-1 '(any . any))))
                                    (if tmp
                                        (apply (lambda (module-name arguments)
                                                 (let* ((module-name (syntax->datum module-name))
                                                        (interface (resolve-interface module-name))
                                                        (select-stx (get-kw-argument arguments identity #:select))
                                                        (hide-stx (get-kw-argument arguments identity #:hide))
                                                        (select (and select-stx (syntax->datum select-stx)))
                                                        (hide (and hide-stx (syntax->datum hide-stx))))
                                                   (if select-stx
                                                       (validate-select-specs module-name interface select-stx))
                                                   (if hide-stx (validate-hide-specs module-name interface hide-stx))
                                                   (loop (cdr specs)
                                                         (append
                                                          (collect-module-bindings
                                                           module-name
                                                           ...
                                                           select
                                                           hide
                                                           (get-kw-argument arguments syntax->datum #:prefix)
                                                           (get-kw-argument arguments identity #:renamer))
                                                          all-bindings))))
                                               tmp)
                                        (syntax-violation #f "source expression failed to match any pattern" tmp-1))))))))
                 (let* ((tmp e) (tmp ($sc-dispatch tmp '(_ each-any any . each-any))))
                   (if (and tmp (apply (lambda (module-spec body body*) (and-map normalize-spec module-spec)) tmp))
                       (apply (lambda (module-spec body body*)
                                (let* ((bindings (collect-all-bindings module-spec))
                                       (ids (map car bindings))
                                       (vals (map cdr bindings))
                                       (labels (gen-labels ids))
                                       (new-w (make-binding-wrap ids labels w))
                                       (new-r (extend-env labels vals r)))
                                  (expand-body (cons body body*) (source-wrap e new-w s mod) new-r new-w mod)))
                              tmp)
                       (syntax-violation 'with-modules "bad with-modules form" (source-wrap e w s mod)))))))
                                            ...
                                            (call-with-values
                                             (lambda ()
                                               (expand-lambda-case
                                                ...
                                                mod
                                                get-formals
                                                (map (lambda (tmp-680b775fb37a463-1f19
                                                              tmp-680b775fb37a463-1f18
                                                              tmp-680b775fb37a463-1f17)
                                                       (cons tmp-680b775fb37a463-1f17
                                                             (cons tmp-680b775fb37a463-1f18 tmp-680b775fb37a463-1f19)))
                                                     e2*
                                                     e1*
                                                     args*)))
                                             (lambda (meta* else*)
                 ...
                 (let* ((tmp-1 e) (tmp ($sc-dispatch tmp-1 '(_ . #(each (any any . each-any))))))
                   (if tmp
                       (apply (lambda (args e1 e2)
                                (build-it
                                 '()
                                 (map (lambda (tmp-680b775fb37a463-217e
                                               tmp-680b775fb37a463-217d
                                               tmp-680b775fb37a463-217c)
                                        (cons tmp-680b775fb37a463-217c
                                              (cons tmp-680b775fb37a463-217d tmp-680b775fb37a463-217e)))
                                      e2
                                      e1
                                      args)))
                              tmp)
                 ...
                 (let* ((tmp-1 e) (tmp ($sc-dispatch tmp-1 '(_ . #(each (any any . each-any))))))
                   (if tmp
                       (apply (lambda (args e1 e2)
                                (build-it
                                 '()
                                 (map (lambda (tmp-680b775fb37a463-21b4
                                               tmp-680b775fb37a463-21b3
                                               tmp-680b775fb37a463-21b2)
                                        (cons tmp-680b775fb37a463-21b2
                                              (cons tmp-680b775fb37a463-21b3 tmp-680b775fb37a463-21b4)))
                                      e2
                                      e1
                                      args)))
                              tmp)
                       (let ((tmp ($sc-dispatch tmp-1 '(_ any . #(each (any any . each-any))))))
                         (if (and tmp (apply (lambda (docstring args e1 e2) (string? (syntax->datum docstring))) tmp))
                             (apply (lambda (docstring args e1 e2)
                                      (build-it
                                       (list (cons 'documentation (syntax->datum docstring)))
                                       (map (lambda (tmp-680b775fb37a463-21ca
                                                     tmp-680b775fb37a463-21c9
                                                     tmp-680b775fb37a463-21c8)
                                              (cons tmp-680b775fb37a463-21c8
                                                    (cons tmp-680b775fb37a463-21c9 tmp-680b775fb37a463-21ca)))
                                            e2
                                            e1
                                            args)))
                                    tmp)
    ...
    (global-extend 'core 'with-modules expand-with-modules)
             ...
             (if tmp-1
                 (apply (lambda (k keyword pattern template)
                          (expand-syntax-rules
                           ...
                           k
                           '()
                           (map (lambda (tmp-680b775fb37a463-24a8 tmp-680b775fb37a463-24a7 tmp-680b775fb37a463-24a6)
                                  (list (cons tmp-680b775fb37a463-24a6 tmp-680b775fb37a463-24a7)
                                        tmp-680b775fb37a463-24a8))
                                template
                                pattern
                                keyword)))
                        tmp-1)
                   ...
                   (if (if tmp-1
                       ...
                       (apply (lambda (k docstring keyword pattern template)
                                (expand-syntax-rules
                                 ...
                                 k
                                 (list docstring)
                                 (map (lambda (tmp-680b775fb37a463-24c1
                                               tmp-680b775fb37a463-24c0
                                               tmp-680b775fb37a463-24bf)
                                        (list (cons tmp-680b775fb37a463-24bf tmp-680b775fb37a463-24c0)
                                              tmp-680b775fb37a463-24c1))
                                      template
                                      pattern
                                      keyword)))
                              tmp-1)
                         ...
                         (if (if tmp-1 (apply (lambda (dots k keyword pattern template) (identifier? dots)) tmp-1) #f)
                             (apply (lambda (dots k keyword pattern template)
                                      (expand-syntax-rules
                                       ...
                                       k
                                       '()
                                       (map (lambda (tmp-680b775fb37a463-24da
                                                     tmp-680b775fb37a463-24d9
                                                     tmp-680b775fb37a463-24d8)
                                              (list (cons tmp-680b775fb37a463-24d8 tmp-680b775fb37a463-24d9)
                                                    tmp-680b775fb37a463-24da))
                                            template
                                            pattern
                                            keyword)))
                                    tmp-1)
                               ...
                               (if (if tmp-1
                                   ...
                                   (apply (lambda (dots k docstring keyword pattern template)
                                            (expand-syntax-rules
                                             ...
                                             k
                                             (list docstring)
                                             (map (lambda (tmp-680b775fb37a463-24f9
                                                           tmp-680b775fb37a463-24f8
                                                           tmp-680b775fb37a463-24f7)
                                                    (list (cons tmp-680b775fb37a463-24f7 tmp-680b775fb37a463-24f8)
                                                          tmp-680b775fb37a463-24f9))
                                                  template
                                                  pattern
                                                  keyword)))
                                          tmp-1)
                                                       ...
                                                       (let ((tmp ($sc-dispatch
                                                         ...
                                                         (if tmp
                                                             (apply (lambda (p)
                                                                      (if (= lev 0)
                                                                          (quasilist*
                                                                           (map (lambda (tmp-680b775fb37a463-25a6)
                                                                                  (list "value"
                                                                                        tmp-680b775fb37a463-25a6))
                                                                                p)
                                                                           (quasi q lev))
                                                                          (quasicons
                                                                           (quasicons
                                                             ...
                                                             (let ((tmp ($sc-dispatch
                                                               ...
                                                               (if tmp
                                                                   (apply (lambda (p)
                                                                            (if (= lev 0)
                                                                                (quasiappend
                                                                                 (map (lambda (tmp-680b775fb37a463-25ab)
                                                                                        (list "value"
                                                                                              tmp-680b775fb37a463-25ab))
                                                                                      p)
                                                                                 (quasi q lev))
                                                                                (quasicons
                                                                                 (quasicons
                                     ...
                                     (let ((tmp ($sc-dispatch
                                       ...
                                       (if tmp
                                           (apply (lambda (p)
                                                    (if (= lev 0)
                                                        (quasilist*
                                                         (map (lambda (tmp-680b775fb37a463-25c1)
                                                                (list "value" tmp-680b775fb37a463-25c1))
                                                              p)
                                                         (vquasi q lev))
                                                        (quasicons
                                                         (quasicons
                                           ...
                                           (let ((tmp ($sc-dispatch
                                             ...
                                             (if tmp
                                                 (apply (lambda (p)
                                                          (if (= lev 0)
                                                              (quasiappend
                                                               (map (lambda (tmp-680b775fb37a463-25c6)
                                                                      (list "value" tmp-680b775fb37a463-25c6))
                                                                    p)
                                                               (vquasi q lev))
                                                              (quasicons
                                                               (quasicons
                          ...
                          (let f ((y x)
                                  (k (lambda (ls)
                                       (let ((tmp-1 ls))
                                         (let ((tmp ($sc-dispatch tmp-1 'each-any)))
                                           (if tmp
                                               (apply (lambda (t-680b775fb37a463-260f)
                                                        (cons "vector" t-680b775fb37a463-260f))
                                                      tmp)
                                               (syntax-violation
                                                #f
                                                "source expression failed to match any pattern"
                            ...
                            (let ((tmp y))
                              (let ((tmp-1 ($sc-dispatch tmp '(#(atom "quote") each-any))))
                                (if tmp-1
                                    (apply (lambda (y)
                                             (k (map (lambda (tmp-680b775fb37a463-261b)
                                                       (list "quote" tmp-680b775fb37a463-261b))
                                                     y)))
                                           tmp-1)
                                      ...
                                      (if tmp-1
                                          ...
                                          (let ((tmp-1 ($sc-dispatch tmp '(#(atom "list*") . #(each+ any (any) ())))))
                                            (if tmp-1
                                                ...
                                                (let ((else tmp))
                                                  (let ((tmp x))
                                                    (let ((t-680b775fb37a463-262a tmp))
                                                      (list "list->vector" t-680b775fb37a463-262a)))))))))))))))))
               (emit (lambda (x)
                       (let ((tmp x))
                         (let ((tmp-1 ($sc-dispatch tmp '(#(atom "quote") any))))
                           (if tmp-1
                                                    ...
                                                    (let f ((x* x))
                                                      (if (null? x*)
                                                          ...
                                                          (let ((tmp-1 (list (emit (car x*)) (f (cdr x*)))))
                                                            (let ((tmp ($sc-dispatch tmp-1 '(any any))))
                                                              (if tmp
                                                                  (apply (lambda (t-680b775fb37a463-264d
                                                                                  t-680b775fb37a463-264c)
                                                                           (list (make-syntax
                                                                                  'cons
                                                                                  '((top))
                                                                                  '(hygiene guile))
                                                                                 t-680b775fb37a463-264d
                                                                                 t-680b775fb37a463-264c))
                                                                         tmp)
                                                                  (syntax-violation
                                                                   #f
