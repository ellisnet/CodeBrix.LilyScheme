...
(define module-gensym
  (let ()
    (define (uid m)
      (number->string (module-generate-unique-id! m) 16))
    (define (mid m)
      (number->string (hash (module-name m) most-positive-fixnum) 16))
    (case-lambda
...
deterministic."
     ;; Note that (current-module) cannot be false at this point; see
     ;; set-current-module above.
     (()
      (let ((m (current-module)))
        (string->symbol (string-append " mg-" (mid m) "-" (uid m)))))
     ((id)
      (let ((m (current-module)))
        (string->symbol (string-append id "-" (mid m) "-" (uid m)))))
     ((id m)
      (if m
          (string->symbol (string-append id "-" (mid m) "-" (uid m)))
          (gensym id))))))
...
(define before-print-with-exp-hook (make-hook 2))
...
(call-with-values (lambda ()
                    ...
                    (let ((syntax? syntax?))
                      ...
                      (values read read-syntax read-bytestring-content)))
  (lambda (read* read-syntax* read-bytestring-content)
    (set! read read*)
    (set! read-syntax read-syntax*)
    (let ((m (save-module-excursion
              (lambda () (define-module (ice-9 read internal))))))
      (module-define! m 'read-bytestring-content read-bytestring-content)
      (module-export! m '(read-bytestring-content)))))
